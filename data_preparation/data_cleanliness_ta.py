...
#
# Usage: python data_cleanliness_ta.py filename
# 
# Description:
# This code measures the cleanliness of our trip advisor data.
# The input is the path to a file containing data collected from trip advisor and processed to generate new features.
...
# regular expression for matching Month, Day, Year format
date_re = '[A-Z][a-z]{2,8} \d{1,2}, \d{4}$'

# each function in score_functions will return False if it detects invalid data for that column
...
score_functions = [                               # conditions for column validity:
    ...
    is_between(10, 100, 'len'),                   # review title 10-100 chars
    ...
    is_between(1, 100),                           # user_helpful_reviews 1-100
    lambda x: bool(re.match(date_re, x)),         # match date format
