...
# Curent version is based on yelp_dc_pilot_2.csv.
...
# Since some of the feature generation is time consuming,
# we first focus on the first few observations. So, this is
# a temporary variable for testing purposes. It should be
...
def make_user_geocode():
    ...
    for i in range(num_rows): 
        try:
            geolocator = Nominatim()
            current_user_geocode = geolocator.geocode(d.loc[i, 'user_location'], timeout=10)
...
if True: # data cleaning
    data_cleaning()
if True: # geocode-based features
    make_restaurant_geocode()
    make_user_geocode()
    make_user_restaurant_distance()
    make_user_is_local()
if True: # review text-based features
    make_user_review_length()
if True: # aggregate mean yelp ratings
...
#print d.head(10)
#print d['user_num_reviews'].head(10)
#print d.tail(10)
...
d.to_csv('data/yelp_dc_pilot_2_added_features.csv', index=False)
