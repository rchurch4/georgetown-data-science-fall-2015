...
// set up svg	
var svg_yelp_ratings = d3.select("#bar_graph_ratings_yelp")
	.append("svg")
...
var current_yelp_graph = "local";

// here we import the data and proceed with callback functions.
d3.tsv("ratings_local_yelp.tsv", type, draw_yelp);	

// finish bar graph
function draw_yelp(error, data) {
	...
	svg_yelp_ratings.selectAll("bar")
		.data(data)
	.enter().append("rect")
		.attr("class", "bar")
...
function update_data_yelp() {
	...
	if (current_yelp_graph=="local") {
		d3.tsv("ratings_nonlocal_yelp.tsv", type, draw_new_yelp);
		d3.select("#title_ratings_yelp").text("Non-Local Reviewers")
		current_yelp_graph="non-local";
	}
	else {
		d3.tsv("ratings_local_yelp.tsv", type, draw_new_yelp);
		d3.select("#title_ratings_yelp").text("Local Reviewers")		
		current_yelp_graph="local";
	}
	
}

// function that transitions to new bar graph
function draw_new_yelp(error, data) {
    ...
    var svg_yelp_ratings = d3.select("#bar_graph_ratings_yelp")  // .transition();
	
	var bars = svg_yelp_ratings.selectAll(".bar")
		.data(data) 
